...
const loadComment = () => {
    ...
    commentsListDiv.innerHTML = commentListArr
        .map((el) => {
            return `
...
const deleteComment = (e) => {
    ...
    for (let i = 0; i < commentsOnStorage.length; i++) {
        if (
            commentDiv.getAttribute('_id') ===
            String(commentsOnStorage[i].comment_id)
        ) {
            const passwordTry = prompt('패스워드를 입력해주세요.');
            if (passwordTry === commentsOnStorage[i].password) {
                ...
                if (
                    confirm(
                        `다음은 삭제하려는 한줄평이 맞습니까?\n\n작성자: ${
                            commentsOnStorage[i].commenter
                        }\n내용: ${
                            commentsOnStorage[i].comment_value
